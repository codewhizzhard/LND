...
export class X {
    constructor(config = {}) {
        this.token = null;
        /** Signup (public) */
        this.signup = async (role, password, email, displayName, orgName, phoneHash, userType) => {
            return await this.auth.signup(role, password, email, displayName, orgName, phoneHash, userType);
        };
        /** Login → automatically sets token */
        this.login = async (role, identifier, password, email) => {
        ...
        this.registerNewCreator = async (publicKey, token, info) => {
        ...
        this.registerExistingCreator = async (accountId, publicKey, signature, challenge, info) => {
