...
export const signup = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      console.error("❌ Guardian invite failed:", (err as Error).message);
...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    if (creator.accountId) payload.accountId = creator.accountId.toString();
    if (creator.creatorDID) payload.creatorDID = creator.creatorDID;
...
/* 
export const registerNewCreator = async (req: Request, res: Response) => {
  try {
    ...
    // WhatsApp flow: verify token and hash phone
    if (token) {
      ...
      // Prevent duplicate registration for WhatsApp users
    ...
    if (!accountId) {
      return res
        .status(500)
        .json({ success: false, error: "Failed to create Hedera account" });
    }

    // Call the full registration logic
...
const tokenStore: Record<string, TokenRecord> = {};

// ---------------------
// Create Token + Link
// ---------------------
export const createWhatsappToken = (
  from: string,
  type: "registration" | "transaction",
  data?: any
) => {
...
export const verifyWhatsappToken = (req: Request, res: Response) => {
  ...
  if (!record) return res.status(400).json({ error: "Expired or invalid token" });

  const phoneHash = hashData(record.phone);
  const response: any = { phoneHash, type: record.type };
...
export const whatsappWebhook = async (req: Request, res: Response) => {
  ...
  if (!from || !body) return res.send("invalid request");
