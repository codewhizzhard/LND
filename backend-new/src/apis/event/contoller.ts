...
import { buildTopicCreateTransaction, createAssetTopic,  executeUserSignedTopicAndMint, sendEventToAsset, tryImmediateConsensusCheck, verifyPublishedMessage } from "../../sdk/index.js";
import type { AuthRequest } from "../auth/auth.js";
import { v4 as uuidv4 } from "uuid";
import type { Event } from "../../model/models.js";
import { Assets } from "../../database/assetSchema.js";
import { Events, EventType, IEvent } from "../../database/eventSchema.js";
...
export const prepareNonScheduleTopicTransaction = async (
  req: AuthRequest,
  res: Response
) => {
  try {
   ...
   /*  if (
    ...
    const txBytes = await buildTopicCreateTransaction(
      userPublicKey,
      userAccountId
    );
...
export const handleCreateAsset = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (metadata.assetTopicId) {
      ...
      if (!newTopic.topicId) throw new Error("Failed to create Hedera topic");
