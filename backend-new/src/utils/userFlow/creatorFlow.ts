...
import type { CidsEvent, Creator, Event } from "../../model/models.js";
...
interface RegisterCreatorData {
  creatorId: string; // now this is the mongoose _id
  accountId: string;
  publicKey: string;
...
export async function registerCreator(data: RegisterCreatorData) {
  try {
   ...
   // const creatorDID = createDID(data.accountId);
    ...
    if (!creatorTopic.topicId) return { success: false, error: "Failed to create creator topic" };
    ...
    if (data.phoneHash) creator.info = { ...creator.info, phoneHash: data.phoneHash };
    if (data.info) creator.info = { ...creator.info, ...data.info };
