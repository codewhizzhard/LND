...
import type { CidsEvent, Creator, CreatorPublic, Event, ICreator } from "../model/models.js";
import { createAccount, createAssetTopic, sendEventToAsset, submitMessageToPrivateTopic } from "../sdk/index.js";
...
export function createDID(accountId: string, network = "testnet") {
...
export function generateAssetId(
    ...
    if (useSalt) base += `-${crypto.randomUUID()}`;
...
/* to verify every action(revoke, verify) */
...
export interface LogVerificationActionParams {
  ...
  topicData: TopicData;
  messageHash: string;
  additionalMessage?: string; // optional extra info
}

export const logVerificationAction = async ({
  targetType,
  targetDid,
  action,
  performedBy,
...
/* hashing data */
export default function sha256hex(obj: any) {
