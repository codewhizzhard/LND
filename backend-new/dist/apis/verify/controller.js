...
export const verifyEventsOnHedera = async (req, res) => {
    try {
        ...
        const hederaEvents = messages.map((msg) => ({
        ...
        const verifiedEvents = await Promise.all(events.map(async (dbE) => {
            let isValid = false;
            let matchedHedera = null;
            for (const he of hederaEvents) {
                try {
                    ...
                    if (hederaHash === dbHash && heTime === dbTime) {
                        ...
                        await Events.updateOne({ _id: dbE._id }, {
                            verified: true,
