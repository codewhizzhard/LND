...
import sha256hex, { logVerificationAction } from "../../utils/utils.js";
...
export const registerIssuer = async (creatorDID, creatorAccountId) => {
    try {
...
//////
export const registerIssuerEcdsaAccount = async (req, res) => {
    try {
        const callerDID = req.creator?.creatorDID;
        if (!callerDID) {
        ...
        if (!evmAddress || evmAddress === null)
            return res.status(500).json({ success: false, error: "account not fully created" });
...
// retrieve
export const retrieveIssuerEcdsaAccount = async (req, res) => {
    try {
        const callerDID = req.creator?.creatorDID;
        if (!callerDID) {
            return res.status(400).json({ success: false, error: "Missing creatorDID" });
...
export const getBusinessesByIssuer = async (req, res) => {
    try {
        const callerDID = req.creator?.creatorDID;
        if (!callerDID) {
            return res.status(400).json({
...
// save transactionId
export const saveTransactionId = async (req, res) => {
    try {
        const creatorDID = req.creator?.creatorDID;
        const callerAccountId = req.creator?.accountId;
        if (!creatorDID) {
...
export const addCreatorType = async (req, res) => {
    try {
        const creatorDID = req.creator?.creatorDID;
        const accountId = req.creator?.accountId;
        if (!creatorDID && !accountId) {
...
export const getBusinessRumors = async (req, res) => {
    try {
        const creatorDID = req.creator?.creatorDID;
        const accountId = req.creator?.accountId;
        if (!creatorDID && !accountId) {
...
export const getIssuers = async (req, res) => {
    try {
        ...
        if (name)
            query["data.name"] = { $regex: new RegExp(name, "i") }; // case-insensitive match
...
export const requestIssuerTrust = async (req, res) => {
    try {
        const creatorDID = req.creator?.creatorDID;
        const accountId = req.creator?.accountId;
...
/// issuer issue trust
export const accessIssuerTrust = async (req, res) => {
    try {
