import { Client, PrivateKey, Hbar, TransferTransaction, TopicCreateTransaction, TopicMessageSubmitTransaction, TopicMessageQuery, AccountCreateTransaction, TopicId, PublicKey, Transaction, AccountId, TransactionId, TransactionRecordQuery, TokenMintTransaction, ContractCreateFlow, ContractFunctionParameters } from "@hashgraph/sdk";
...
export const createAssetTopic = async (memo) => {
    if (!memo)
        throw new Error("Memo is required to create topic");
...
export async function sendEventToAsset(data) {
    let submitMsgTx;
...
export async function getAssetEvent(topicId) {
    ...
    (message, error) => {
        ...
        if (message) {
            ...
            const msgBuffer = Buffer.from(message.contents.buffer, message.contents.byteOffset, message.contents.byteLength);
...
export async function getEventInfo(number, topicId) {
...
export async function buildTopicCreateTransaction(userPublicKey, userAccountId, memo = "User-owned topic with auto-renew") {
    // Build transaction
...
/// non schedule
export async function executeUserSignedTopicAndMint(signedTopicTxBytesBase64, signedMintTxBytesBase64) {
    try {
        ...
        const topicTx = TopicCreateTransaction.fromBytes(Uint8Array.from(Buffer.from(signedTopicTxBytesBase64, "base64")));
        ...
        if (signedMintTxBytesBase64) {
            const mintTx = Transaction.fromBytes(Uint8Array.from(Buffer.from(signedMintTxBytesBase64, "base64")));
 ...
 */
export async function verifyPublishedMessage(topicId, expectedHash, consensusTimestamp, mirrorUrl = "https://testnet.mirrornode.hedera.com") {
    try {
 ...
 */
export async function tryImmediateConsensusCheck(transactionId, retries = 5, delayMs = 2000) {
    try {
        ...
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                ...
                console.log(`⌛ Attempt ${attempt}: Transaction not yet in consensus (status: ${record.receipt.status})`);
            }
            catch (err) {
...
export async function verifyPendingEvents() {
    ...
    for (const ev of pendingEvents) {
        try {
            ...
            if (record.receipt.status.toString() !== "SUCCESS") {
                console.log(`⚠️ Event ${ev.eventId} still not successful (status: ${record.receipt.status})`);
                continue;
            }
            const consensusTimestamp = record.consensusTimestamp?.toDate();
            await Events.updateOne({ eventId: ev.eventId }, {
                consensusTimestamp,
                verified: true,
            });
            console.log(`🎉 Event ${ev.eventId} verified @ ${consensusTimestamp}`);
        }
        catch (err) {
            console.error(`❌ Error verifying event ${ev.eventId}:`, err.message);
        }
    }
}
async function transferHbar(senderAccountId, senderPrivateKey, receiverAccountId) {
    try {
...
async function getAccessToken(token) {
    ...
    try {
        const res = await axios.post(`${GUARDIAN_BASE_URL}/accounts/access-token`, { refreshToken: tokenvalue }, { headers: { "Content-Type": "application/json" } });
...
export async function invitePlatformUser(accessToken, email, type) {
    try {
        const userInvite = {
            email,
        ...
        const res = await axios.post(`${GUARDIAN_BASE_URL}/tenants/invite`, userInvite, {
            headers: {
...
export async function inviteUser(email, type) {
    try {
...
export async function getId(accessToken, email, username) {
    try {
        const data = {
            email,
            username
        };
        const res = await axios.post(`${GUARDIAN_BASE_URL}/tenants/${TENANT_ID}/users`, data, {
            headers: {
                Authorization: `Bearer ${accessToken}`,
                "Content-Type": "application/json",
                "x-tenant-id": TENANT_ID,
...
export async function mgsLogin(password, userId, email) {
    ...
    try {
        ...
        const res = await axios.post(`${GUARDIAN_BASE_URL}/accounts/loginByEmail`, data, {
            headers: {
                "Content-Type": "application/json",
                "x-tenant-id": TENANT_ID,
            },
...
export async function profile(access, username) {
    try {
        const res = await axios.get(`${GUARDIAN_BASE_URL}/profiles/${username}`, {
            headers: {
...
export const getProfileData = async (email, username, password) => {
    try {
        const accessToken = await getAccessToken();
...
export async function submitMessageToPrivateTopic(message) {
    ...
    try {
        ...
        console.log(`✅ Message submitted to topic ${VERIFICATION_TOPIC_ID}. Status: ${status}`);
        if (status !== "SUCCESS") {
            return {
                success: false,
...
export async function mintAndTransferNFT(recipientId) {
    ...
    try {
        ...
        if (!serial)
            throw new Error("No NFT serials were minted");
...
export async function generateEdscaAccount(publicKey) {
    try {
        if (!publicKey) {
            return { success: false, error: "Public key is required to create account" };
        }
...
export async function deployIssuerBondManager() {
    try {
        ...
        const contractCreateTx = new ContractCreateFlow()
            ...
            .setConstructorParameters(new ContractFunctionParameters().addAddress(maintenanceWallet))
        ...
        if (!contractId)
            return { success: false, error: "error getting contractId" };
...
async function getContractActions(txId, network) {
...
export async function issueVcJwtForBusiness(issuerDid, business) {
