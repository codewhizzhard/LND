...
export function useEcdsaWallet({ autoLockMinutes = 5, storageKey = STORAGE_KEY, } = {}) {
    const [isInitialized, setIsInitialized] = useState(false);
    const [isUnlocked, setIsUnlocked] = useState(false);
    const [publicKey, setPublicKey] = useState(null);
    const [privateKeyStr, setPrivateKeyStr] = useState(null); // optional viewable string
    ...
    const viewPrivateKey = useCallback(async (password) => {
    ...
    const importKeystore = useCallback(async (keystoreJson, password) => {
        const parsed = typeof keystoreJson === "string"
    ...
    // ---------- sign raw bytes ----------
    const signBytes = useCallback(async (data) => {
    ...
    const signTransaction = useCallback(async (tx) => {
        ...
        try {
            const isFrozen = tx.isFrozen ? tx.isFrozen() : undefined;
    ...
    return {
        ...
        signBytes,
        signTransaction,
        setAutoLockMinutes: (m) => setAutoLock(m),
    };
}
/*
