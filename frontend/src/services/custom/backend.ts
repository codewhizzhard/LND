 ...
 */
export async function getAccountIdFromBackend(publicKey: string, phoneHash?: string): Promise<string | null> {
    try {
        ...
        if (!res.ok) throw new Error(`Backend GET account failed: ${res.statusText}`);
 ...
 */
export async function createAccountOnBackend(publicKey: string, phoneHash?: string): Promise<string> {
    try {
        ...
        if (!res.ok) throw new Error(`Backend CREATE account failed: ${res.statusText}`);

        const data: BackendResponse = await res.json();
        if (!data.accountId) throw new Error("No accountId returned from backend");
...
/**
 * Utility function to get or create an account on the backend
 * @param publicKey Hex string of user's Hedera public key
 * @returns accountId
 */

// services/custom/backend.ts
interface CrossProjectArgs {
  publicKey: string;
  signature?: string;   // optional for self-custodian
  challenge?: string; 
  phoneHash?: string | null // optional for self-custodian
}
...
const verifyAndGetAccountId = async (publicKey: string, signature: string, challenge: string) => {
...
export async function getOrCreateAccountOnBackend({
  publicKey,
  signature,
  challenge,
 
}: CrossProjectArgs): Promise<string> {
  ...
  if (signature && challenge) {
    ...
    if (verifiedAccountId) return verifiedAccountId;
  }
