...
export async function importKeystore(password, keystoreOrString) {
    const parsed = typeof keystoreOrString === "string"
        ? JSON.parse(keystoreOrString)
        : keystoreOrString;
    if (parsed.version !== 1)
 ...
 */
export function generateHederaEcdsaAccount() {
 ...
 */
export function encryptPrivateKey(privateKey, userSecret) {
 ...
 */
export function decryptPrivateKey(encryptedPrivateKey, userSecret, saltB64, ivB64) {
    try {
