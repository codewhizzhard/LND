import { PrivateKey, TopicCreateTransaction, Client, ScheduleSignTransaction, ScheduleId, AccountId, TopicMessageSubmitTransaction, TopicId, ScheduleCreateTransaction, } from "@hashgraph/sdk";
/**
 ...
 */
export async function signTopicTransaction(txBytesBase64, userPrivateKey) {
...
export async function signScheduledTopic(scheduleIdStr, operatorKey, // private key in string form
operatorId // account id in string form ("0.0.xxxx")
) {
    // Parse Hedera types
...
export async function createNonScheduleMessageToTopic(counterpartyId, counterpartyKey, topicId, message) {
    // Initialize client
    const client = Client.forTestnet().setOperator(AccountId.fromString(counterpartyId), PrivateKey.fromString(counterpartyKey));
    try {
...
export async function createScheduledMessageTransaction(counterpartyId, counterpartyKey, topicId, message) {
    const client = Client.forTestnet().setOperator(AccountId.fromString(counterpartyId), PrivateKey.fromString(counterpartyKey));
    try {
