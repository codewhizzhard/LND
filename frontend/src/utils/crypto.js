...
export async function deriveKeyFromPassword(password, salt) {
    ...
    const baseKey = await crypto.subtle.importKey("raw", enc.encode(password), // BufferSource
...
export async function encryptWithPassword(password, plaintext) {
    ...
    const cipherBuffer = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, // safe Uint8Array
    key, data);
    return {
...
export async function decryptWithPassword(password, payload) {
    ...
    const key = await deriveKeyFromPassword(password, salt);
    console.log("key:", key);
    const plainBuffer = await crypto.subtle.decrypt({ name: "AES-GCM", iv }, // iv is now a proper Uint8Array
    key, cipherBytes);
